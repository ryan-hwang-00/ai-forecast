...
def predictor(merged_df, store_code=6, product_name='백산수500ml', predict_date='2020-01-07'):
    ...
    x_scaler, x_1_scaler, y_scaler, column_num_x, column_num_x_1, x_columns, x_1_columns, sale_qty = data.scaled_origin(sequence_x=180 * 4,
                                                                                                                        sequence_y=7,
                                                                                                                        product_name=product_name,
                                                                                                                        store_code=store_code)

    x_test_scaled, x_test_1_scaled, y_test_scaled = data.scaled_data(sequence_x=180 * 4,
                                                                     sequence_y=7,
                                                                     df_train=df_test,
                                                                     product_name=product_name,
                                                                     store_code=store_code
